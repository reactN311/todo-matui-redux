...
export const reducerTodos = produce(( state: ITodoState = initState, action: TAction ) => {
  switch (action.type) {
    ...
    case ActionType.COMPLETED_TODO:
      state.dataTodo[action.payload].completed = !state.dataTodo[action.payload].completed
      return
    default:
      return state
  }
